...
class App(Tk):
    ...
    def __init__(self):
        ...
        # 标题下分割线
        title_sep = Frame(self, height=1, bg=BORDER)
        title_sep.grid(row=1, column=0, sticky="ew")
        self.rowconfigure(2, weight=1)

        # ── 主 Notebook（策略回测 / 实盘策略 标签页切换）──
        main_style = ttk.Style()
        main_style.theme_use("clam")
        main_style.configure("Main.TNotebook", background=BG_PRIMARY, borderwidth=0)
        main_style.configure("Main.TNotebook.Tab", background=BG_TERTIARY, foreground=FG_SECONDARY,
                             padding=[16, 4], font=("Microsoft YaHei", 10))
        main_style.map("Main.TNotebook.Tab", background=[("selected", BG_SECONDARY)],
                       foreground=[("selected", FG_PRIMARY)])
        ...
        # ── 标签页1：策略回测 ──
        backtest_tab = Frame(main_notebook, bg=BG_PRIMARY)
        main_notebook.add(backtest_tab, text="📊 策略回测")
        backtest_tab.rowconfigure(0, weight=1)
        backtest_tab.columnconfigure(0, weight=0)  # left: 策略列表
        backtest_tab.columnconfigure(1, weight=1)  # right: 详情面板

        # 左侧：策略列表
        left_frame = Frame(backtest_tab, bg=BG_PRIMARY)
        left_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 5), pady=(0, 0))

        header_frame = Frame(left_frame, bg=BG_PRIMARY)
        header_frame.pack(fill=X, pady=(0, 4))
        Label(header_frame, text="策略列表", font=("Microsoft YaHei", 12, "bold"),
              fg=ACCENT_BLUE, bg=BG_PRIMARY).pack(side=LEFT)
        refresh_btn = Button(header_frame, text="↻", font=("Microsoft YaHei", 11, "bold"),
        ...
        balance_btn = Button(header_frame, text="💰", font=("Segoe UI Emoji", 12),
                             fg=ACCENT_GOLD, bg=BG_TERTIARY, bd=0, padx=6, pady=0,
                             activebackground=BG_ELEVATED, activeforeground=ACCENT_GOLD,
                             command=self.open_balance, cursor="hand2",
                             relief="flat", highlightthickness=0)
        balance_btn.pack(side=RIGHT, padx=(0, 2))

        # 运行按钮
        run_bar = Frame(left_frame, bg=BG_PRIMARY)
        run_bar.pack(fill=X, pady=(0, 4))
        self.run_all_btn = Button(run_bar, text="▶ 全量回测",
                                 font=("Microsoft YaHei", 10),
                              fg=FG_PRIMARY, bg=BTN_TEAL, bd=0, padx=8, pady=3,
                              activebackground=ACCENT_TEAL_DIM, activeforeground=FG_PRIMARY,
                                  command=self.run_all, cursor="hand2")
        self.run_all_btn.pack(side=LEFT, fill=X, expand=True, padx=(0, 3))
        self.run_sel_btn = Button(run_bar, text="▶ 运行选中",
                                 font=("Microsoft YaHei", 10),
                                 fg=FG_PRIMARY, bg=BTN_BLUE, bd=0, padx=8, pady=3,
                                 activebackground=ACCENT_BLUE_DIM, activeforeground=FG_PRIMARY,
                                  disabledforeground=FG_GHOST,
                                  command=self.run_selected, cursor="hand2")
        self.run_sel_btn.pack(side=LEFT, fill=X, expand=True, padx=(3, 0))
        self.run_sel_btn.config(state="disabled")

        # 自动研发按钮
        self.auto_btn = Button(run_bar, text="🤖 自动研发",
                               font=("Microsoft YaHei", 10),
                               fg=FG_PRIMARY, bg=BTN_PURPLE, bd=0, padx=8, pady=3,
                               activebackground=ACCENT_PURPLE_DIM, activeforeground=FG_PRIMARY,
                               disabledforeground=FG_GHOST,
                               command=self.auto_develop, cursor="hand2")
        self.auto_btn.pack(side=LEFT, fill=X, expand=True, padx=(3, 0))
        self.stop_btn = Button(run_bar, text="⏹ 停止",
                               font=("Microsoft YaHei", 10),
                              fg=FG_PRIMARY, bg=BTN_RED, bd=0, padx=8, pady=3,
                              activebackground=ACCENT_RED_DIM, activeforeground=FG_PRIMARY,
                               disabledforeground=FG_GHOST,
                               command=self.stop_dev, cursor="hand2")
        self.stop_btn.pack(side=LEFT, padx=(3, 0))
        self.stop_btn.config(state="disabled")

        self.tree = ttk.Treeview(left_frame, columns=("name", "ret", "csi_sharpe", "eq_sharpe", "turnover", "dd", "created"),
        ...
        self.tree.tag_configure("live", background="#1a3a1a")

        for col_id, col_text, col_width, _, _ in COLUMNS:
            self.tree.heading(col_id, text=col_text,
                              command=lambda c=col_id: self.sort_by(c))
            self.tree.column(col_id, width=col_width, minwidth=col_width)
            if col_id != "name":
                self.tree.column(col_id, anchor="center")
        ...
        # 右侧：详情面板
        right_frame = Frame(backtest_tab, bg=BG_SECONDARY, bd=1, relief="solid",
                            highlightbackground=BORDER, highlightthickness=1)
        right_frame.grid(row=0, column=1, sticky="nsew", padx=(5, 0), pady=(0, 0))
        right_frame.rowconfigure(2, weight=1)
        right_frame.columnconfigure(0, weight=1)

        # ── 标签页2：实盘策略 ──
        live_tab = Frame(main_notebook, bg=BG_SECONDARY)
        main_notebook.add(live_tab, text="🔴 实盘策略")
        ...
        # ══════════════════════════════════════════════════
        # 第1个 Pane：实盘策略列表（无合计条）
        # ══════════════════════════════════════════════════
        strat_sec = make_section(pw)
        strat_header = Frame(strat_sec, bg=BG_SECONDARY)
        strat_header.grid(row=0, column=0, columnspan=2, sticky="ew")
        Label(strat_header, text="📋 实盘策略", font=("Microsoft YaHei", 12, "bold"),
              fg=FG_PRIMARY, bg=BG_SECONDARY).pack(side=LEFT)
        live_refresh_btn = Button(strat_header, text="↻", font=("Microsoft YaHei", 11, "bold"),
                                  fg=FG_PRIMARY, bg=BG_TERTIARY, bd=0, padx=8, pady=0,
                                  activebackground=BG_ELEVATED, activeforeground=FG_PRIMARY,
                                  command=self.refresh_live, cursor="hand2",
                                  relief="flat", highlightthickness=0)
        live_refresh_btn.pack(side=RIGHT, padx=(4, 0))

        STRAT_COLS = [
            ("name", "策略名称", 160), ("status", "状态", 80),
        ...
        for cid, ctext, cwidth in STRAT_COLS:
            self.strat_tree.heading(cid, text=ctext)
            self.strat_tree.column(cid, width=cwidth, minwidth=cwidth, anchor="center")
        self.strat_tree.column("name", anchor="w")
        ...
        pos_header = Frame(pos_sec, bg=BG_SECONDARY)
        pos_header.grid(row=0, column=0, columnspan=2, sticky="ew")
        Label(pos_header, text="📊 组合持仓（等比缩仓）", font=("Microsoft YaHei", 12, "bold"),
              fg=FG_PRIMARY, bg=BG_SECONDARY).pack(side=LEFT)

        POS_COLS = [
            ("code", "股票代码", 120), ("pname", "股票名称", 105),
        ...
        self.pos_tree = ttk.Treeview(pos_sec,
            ...
            show="headings", height=5, selectmode="browse")
        for cid, ctext, cwidth in POS_COLS:
            self.pos_tree.heading(cid, text=ctext)
            self.pos_tree.column(cid, width=cwidth, minwidth=cwidth, anchor="center")
        self.pos_tree.column("pname", anchor="w")
        ...
        SP_COLS = [
            ...
            ("price", "参考价", 90), ("amount", "占用资金", 100),
            ("pct", "占比", 75),
        ]
        self.sp_tree = ttk.Treeview(sp_sec,
            columns=("code", "pname", "direction", "lots", "price", "amount", "pct"),
            show="headings", height=5, selectmode="browse")
        for cid, ctext, cwidth in SP_COLS:
            self.sp_tree.heading(cid, text=ctext)
            self.sp_tree.column(cid, width=cwidth, minwidth=cwidth, anchor="center")
        self.sp_tree.column("pname", anchor="w")
        self.sp_tree.column("direction", anchor="center")
        ...
        self.dev_frame.grid_remove()  # 默认隐藏

        self.detail_title = Label(right_frame, text="选择左侧策略查看详情",
                                  font=("Microsoft YaHei", 14, "bold"),
                                  fg=FG_PRIMARY, bg=BG_SECONDARY, pady=10, padx=12,
                                  anchor="w")
        self.detail_title.grid(row=1, column=0, sticky="ew")
        # 标题下分割线
        sep = Frame(right_frame, height=1, bg=BORDER)
        ...
        style2.configure("TNotebook.Tab", background=BG_TERTIARY, foreground=FG_PRIMARY,
                        padding=[16, 4], font=("Microsoft YaHei", 10, "bold"))
        ...
        # 代码工具栏
        code_toolbar = Frame(code_tab, bg=BG_SECONDARY)
        code_toolbar.grid(row=0, column=0, columnspan=2, sticky="ew", pady=(4, 0))
        self.save_btn = Button(code_toolbar, text="💾 保存",
                               font=("Microsoft YaHei", 10),
                                bg=BTN_TEAL, fg=FG_PRIMARY, bd=0, padx=12, pady=2,
                                activebackground=ACCENT_TEAL_DIM, activeforeground=FG_PRIMARY,
                               command=self.save_code, cursor="hand2")
        self.save_btn.pack(side=LEFT, padx=(4, 4))
        self.revert_btn = Button(code_toolbar, text="↩ 撤销修改",
                                 font=("Microsoft YaHei", 10),
        ...
        self.code_status = Label(code_toolbar, text="",
        ...
        # 语法高亮标签
        self.code_text.tag_configure("kw", foreground=ACCENT_PURPLE)
        self.code_text.tag_configure("str", foreground=ACCENT_GOLD)
        self.code_text.tag_configure("cmt", foreground=FG_MUTED)
        self.code_text.tag_configure("num", foreground=ACCENT_PURPLE)
        self.code_text.tag_configure("dec", foreground=ACCENT_TEAL)
        self.code_text.tag_configure("bif", foreground=ACCENT_BLUE)
        ...
        # 底部：指标 + 按钮
        btn_frame = Frame(right_frame, bg=BG_SECONDARY)
        btn_frame.grid(row=3, column=0, sticky="ew", pady=6)

        self.stats_text = Text(btn_frame, height=3, font=("Consolas", 10),
        ...
        self.stats_text.tag_configure("stat_val_pos", foreground=ACCENT_TEAL)
        self.stats_text.tag_configure("stat_val_neg", foreground=ACCENT_RED)
        self.stats_text.tag_configure("stat_val", foreground=ACCENT_BLUE)
        ...
        self.strategies.sort(
            key=lambda item: self._get_sort_key(self.sort_col, item),
    ...
    def auto_develop(self):
        ...
        self.dev_text.insert("1.0", "正在启动 Hermes 后台进程...\n")
